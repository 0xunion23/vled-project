...
import { generateWithOllama, validateWithOllama, detectHallucination } from "./ollamaService.js";
...
export async function answerQuestion(query) {
  ...
  if (isFaqAnswer(answer)) {
    const isFake = await detectHallucination({ contexts, answer });
    if (isFake === 'fake') {
      console.warn(`[Accuracy Alert] Hallucination detected! Query: "${normalizedQuery}", Answer: "${answer}"`);
      try {
        await SecurityLog.create({
          payload: `[Query]: ${normalizedQuery}\n[Hallucinated Response]: ${answer}`,
          threatLevel: 'Medium',
          detectedPattern: 'Hallucination Detected',
          blockedReason: 'Ungrounded or fabricated information generated by LLM'
        });
      } catch (err) {
        console.error('Failed to save hallucination log to MongoDB:', err);
      }

      return {
        answer: 'Accuracy Alert: The generated answer failed fact-check verification (potential hallucination). This response has been blocked for reliability.',
        answerFound: false,
        confidence: bestScore,
        sources: results.map(toSource),
        blocked: true,
        warning: 'Blocked due to hallucination detection.'
      };
    }
  }
